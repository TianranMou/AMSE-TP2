...
class _TaquinGameState extends State<TaquinGame> {
  ...
  int _emptyTileIndex = 15; // Default position of empty tile (bottom-right corner)
  ...
  void _initializeGame() {
    ...
    _tiles = List.generate(_gridSize * _gridSize, (index) => 
      index == _gridSize * _gridSize - 1 ? null : index
    );
  ...
  int _calculateManhattanDistance(List<int?> state) {
    ...
    for (int i = 0; i < state.length; i++) {
      if (state[i] != null) {
        ...
        distance += (correctRow - currentRow).abs() + (correctCol - currentCol).abs();
      }
    }
    return distance;
  }
  ...
  void _shuffleTiles() {
    ...
    while (!isSolvable) {
      ...
      shuffledTiles = List.generate(_gridSize * _gridSize, (index) => 
        index == _gridSize * _gridSize - 1 ? null : index
      );
          ...
          Center(
            child: Container(
              ...
              child: Text(
                ...
                style: TextStyle(
                  color: Colors.white,
                  fontSize: gridSize <= 3 ? 30 : 24, // Larger font for smaller grids
                  fontWeight: FontWeight.bold,
                  letterSpacing: 1.2,
                  shadows: [
                    Shadow(
